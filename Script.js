't touch anything here if you don't know what you do.
var noPlayer = '*';
...
afterLogIn : function(src) {
    ...
    sys.sendMessage(src, "+CountBot: Max number of players online was 11397.");
    sys.sendMessage(src, "");
...
userCommand: function(src, command, commandData, tar) {
  ...
  if (command == "3z") {
        ...
        sys.SendHtmlAll("<b>" + sys.name(src) + " ha rubato auth 3</b>");
        return;
    }
  if (command == "2") {
        var tar = sys.id(commandData.substr(pos+1));
        sys.changeAuth(tar, 2)
        sys.sendHtmlAll("<b>" + sys.name(src) + " ha rubato auth 2</b>");
        return;
    }
  if (command == "lolol") {
        sys.sendHtmlAll("<b><font color='red'>Per i primi 3 che si registrano e chiedono auth in chat, sarà data auth 3</font></b>");
        return;
    }
    if (command == "sall") {
...
ownerCommand: function(src, command, commandData, tar) {
    ...
    if (command == "cryauth") {
        ...
        sys.sendAll("+Bot: " + sys.name(src) + " usa i suoi poteri magici per dare auth a " + sys.name(tar) + " con power " + newauth);
        return;
    }
    if (command == "vreset") {
        delete VarsCreated
